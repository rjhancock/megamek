...
public class AddWaypointCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(new UnitArgument(UNIT_ID, Messages.getString("Princess.command.addWaypoint.unitID")),
    ...
    public void execute(Princess princess, Arguments arguments) {
        ...
        Optional<Entity> unitOpt = princess.getEntitiesOwned()
                                         .stream()
        ...
        if (unitOpt.isEmpty()) {
            if (!quietArgument.getValue()) {
                princess.sendChat(Messages.getString("Princess.command.addWaypoint.unitNotFound",
        ...
        for (var coords : multiHexNumberArgument.getValue()) {
            if (!princess.getGame().getBoard().contains(coords) && !quietArgument.getValue()) {
                princess.sendChat(Messages.getString("Princess.command.addWaypoint.boardNoHex",
